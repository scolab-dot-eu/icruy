...
@section('content')
    ...
    <h2>{{ __('Capas base y de apoyo') }}</h2>
    <table class="table table-striped table-bordered">
        ...
        <tbody>
        @foreach($supportlayerdefs as $value)
            <tr>
                <td>{{ $value->id }}</td>
                <td>{{ $value->name }}</td>
                <td>{{ $value->title }}</td>
    ...
    <a class="btn btn-small btn-info" href="{{ URL::to('dashboard/supportlayerdefs/create') }}">{{__('Nueva capa') }}</a>
...
<script type="text/javascript">
    $( document ).ready(function() {
        $('#deleteModal').on('show.bs.modal', function (event) {
              ...
              var action = '/dashboard/supportlayerdefs/' + id;
